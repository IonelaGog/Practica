...
template <typename TValue>
void Vector<TValue>::insert(TIterator pos, const TValue &element)
{
    ...
    for (TIterator current = pos + 1; current != end(); ++current)
    {
        *current = *(current - 1);
    }
...
template <typename TValue>
void Vector<TValue>::pushBack(const TValue &element)
{
...
{
    this->m_data[m_size++] = std::move(element);
}
...
void Vector<TValue>::reserve(std::size_t newCapacity)
{
    if (this->m_capacity < newCapacity)
    {
        ...
        pdv::move(m_data, m_data + m_size, new_data);
